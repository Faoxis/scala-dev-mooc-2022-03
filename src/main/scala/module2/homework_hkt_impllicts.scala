...
object homework_hkt_impllicts{
  ...
  implicit def optionToBindable[A](option: List[A]): Bindable[List, A] =
    new Bindable[List, A] {
      override def map[B](f: A => B): List[B] = option.map(f)
      override def flatMap[B](f: A => List[B]): List[B] = option.flatMap(f)
    }
